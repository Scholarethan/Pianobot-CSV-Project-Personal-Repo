...
void Hand::addHandPos(int time, int position)
{
	...
	// Reset state number because states are empty for the new position and increment position index 
	stateNum = -1;
	posNum++;
}
...
void Hand::addState(int time, int finger, bool onOff)
{
	...
	for (int i = 0; i < 16; i++)
	{
		//if it's not the finger that's being changed, copy everything from before. 
		if (i != finger)
		{
			//if stateNum == -1 must mean it's the first hand position or a new hand position, so there can't be any fingers being pressed
			if (stateNum == -1)
			{
		...
		//If it is the finger that's being changed, then you change it
		else
		{
...
// Checks how far do I need to move for a note in semitones from end of hand
...
int Hand::handMoveDist(int note)
{
	...
	// Get relative position (removing octaves) - thumb position in semitones
	...
	// Used to store # of semitone movements needed
	int toMove;

	// Used to determine number of semitones to move
	// First index is hand's relative white key position (THUMB CANT BE ON A BLACK KEY, THATS WHY 7 IN FIRST INDEX FOR 7 WHITE KEYS IN AN OCTAVE)
	// Second index is the note' relative position relative to C
	int moveRef[7][12] =
	{
		//Semitone distance of relative note position (note that thumb is on) - relative
		//(note that thumb is on) thumb position [(note - thumb position) % 12 to remove negatives]
	...
	//(thumb distance in white keys away from C)
	int relativeHandPos[12] = { 0, -1, 1, -1, 2, 3, -1, 4, -1, 5, -1, 6 };

	// Calculate relative hand position (thumb) of white keys
...
//TELLS YOU WHAT FINGER A NOTE IS ASSOCIATED WITH ON THE HAND
...
int Hand::findFinger(int note)
{
	...
	// Get thumb position (between 0 - 11) remember, this is in semitones
	int relativePos = currentPos % 12;

	// Calculate relative note position (removing octaves)
	int relativeNotePos = note % 12;

	// Calculate difference in position between note and thumb position in semitones
	int diffPos = note - currentPos;

	// Used to figure out if note is white or black
	bool isBlack[12] = { 0, 1, 0, 1, 0, 0, 1, 0, 1, 0, 1, 0 };

	// First index is hand's thumb white key position
	// Second index is the note's position relative to the thumb
	int moveRef[7][14] =
	{
	...
	// Used to figure out which white key the thumb is on for moveReference
	int relativeHandPos[12] = { 0, -1, 1, -1, 2, 3, -1, 4, -1, 5, -1, 6 };

	// Calculates which white key the thumb is on
	int handPosIdx = relativeHandPos[relativePos];
